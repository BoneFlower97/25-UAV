...
uint8_t sendbuff[32];
uint8_t sendflag = 0;
static uint8_t test_buff[32];
pid_group_t pid_group;
...
static void receiveWirelessData(uint8_t *payload, uint16_t size)
{
  ...
  uint16_t    YAW = 30;
	...
	if(size>5)
	{
				...
				{
					case 0xE0:
						if(payload[6] == 0x01) con.motor_lock = true;
						else if(payload[6] == 0x00) con.motor_lock = false;
					...
					case 0xE1:
						...
						else if(PAR_ID>=11&&PAR_ID<=28)PAR_VAL = pid_group.pid_array[PAR_ID-11];
						else PAR_VAL = 0xFF;
					...
					case 0xE2:
						PAR_ID  = ((uint16_t)payload[5] << 8) | payload[4];	
						if(PAR_ID>=11&&PAR_ID<=28) pid_group.pid_array[PAR_ID-11] = ((uint16_t)payload[7] << 8) | payload[6];
						sendbuff[step++] = 0X00;
						sendbuff[step++] = 0X03;
						sendbuff[step++] = payload[2];
						sendbuff[step++] = payload[10];
						sendbuff[step++] = payload[11];
						sendflag = 1;
					break;
					case 0x41:
						rc_ctrl.ctrl_rol = (int16_t)((payload[5] << 8) | (payload[4]));
						rc_ctrl.ctrl_pit = (int16_t)((payload[7] << 8) | (payload[6]));
						rc_ctrl.ctrl_thr = (int16_t)((payload[9] << 8) | (payload[8]));
						rc_ctrl.ctrl_yawdps = (int16_t)((payload[11] << 8) | (payload[10]));
						if(payload[12] == 0x01) 
						{
								con.motor_lock = true;
						}
						else if(payload[12] == 0x00 )
						{
					...
					if(xQueueReceive(stateDataQueue, &stateToSend, 0) == pdTRUE && (switch_send %10) ==0)
					{
						...
						sendbuff[step++] = 0x03;
						sendbuff[step++] = 7;
						sendbuff[step++] = (int8_t)(ROL & 0xFF);
						sendbuff[step++] = (int8_t )((ROL >> 8) & 0xFF);
						sendbuff[step++] = (int8_t )(PIT & 0xFF);
						sendbuff[step++] = (int8_t )((PIT >> 8) & 0xFF);
						sendbuff[step++] = (int8_t )(YAW & 0xFF);
						sendbuff[step++] = (int8_t )((YAW >> 8) & 0xFF);
						sendbuff[step++] = 1;
						sendflag = 1;
					
					}
					else if(xQueueReceive(sensorDataQueue, &sensorDataToSend, 0) == pdTRUE && (switch_send %5) ==0)
					{
						acc_x = (int16_t )(sensorDataToSend.acc.x*100);
						acc_y = (int16_t )(sensorDataToSend.acc.y*100);
						acc_z = (int16_t )(sensorDataToSend.acc.z*100);
						gyro_x = (int16_t )(sensorDataToSend.gyro.x*1000);
						gyro_y = (int16_t )(sensorDataToSend.gyro.y*1000);
						gyro_z = (int16_t )(sensorDataToSend.gyro.z*1000);
						sendbuff[step++] = 0x01;
						sendbuff[step++] = 13;
						sendbuff[step++] = (int8_t)(acc_x & 0xFF);
						sendbuff[step++] = (int8_t)((acc_x >> 8) & 0xFF);
						sendbuff[step++] = (int8_t)(acc_y & 0xFF);
						sendbuff[step++] = (int8_t)((acc_y >> 8) & 0xFF);
						sendbuff[step++] = (int8_t)(acc_z & 0xFF);
						sendbuff[step++] = (int8_t)((acc_z >> 8) & 0xFF);
						sendbuff[step++] = (int8_t)(gyro_x & 0xFF);
						sendbuff[step++] = (int8_t)((gyro_x >> 8) & 0xFF);
						sendbuff[step++] = (int8_t)(gyro_y & 0xFF);
						sendbuff[step++] = (int8_t)((gyro_y >> 8) & 0xFF);
						sendbuff[step++] = (int8_t)(gyro_z & 0xFF);
						sendbuff[step++] = (int8_t)((gyro_z >> 8) & 0xFF);					
						sendbuff[step++] = 1;	
						sendflag = 1;
						
					}
					else Radio.Rx( FSK_RX_TIMEOUT_VALUE );
...
void wirelessTask(void *param){
	...
	while(1){
		...
		//delay_ms(1);
//		if(global_usart.rx_flag == 1)
//		{
//			Radio.Standby();
//			//Radio.Send((uint8_t *)global_usart.UART1_RxBuffer,global_usart.rx_len);
//			Radio.Send((uint8_t *)global_usart.UART1_RxBuffer,FSK_PAYLOAD_LEN);
//			global_usart.rx_flag = 0;
//			
//		}
//		if(global_usart.tx_flag == 1)
//		{
//			Radio.Standby();
//			Radio.Send((uint8_t *)sendbuff,FSK_PAYLOAD_LEN);;
//			global_usart.tx_flag = 0;	
//			//Radio.Rx( FSK_RX_TIMEOUT_VALUE );
//		
//		}
	}
